...
def zastempstwa_u(klasa,res_text,data):
    ...
    if tabela is None:
        ...
        temp = [data, klasa_tabela]
        zast_data.append(temp)
...
def zast_and_plan(tekst):
    ...
    for i, day in enumerate(dates):
        if day.content[-8::] != b'readable':
            if case == 0:
                dates[i] = zastempstwa_u(klasa, day.text, daty[i])
            elif case == 1:
        ...
        if day != "None" and day != 'XXX':
            for lekcja in day:
                if lekcja[0] != '' and int(lekcja[0]) > 0:
                    ...
                    lekcja = [lekcja_temp, [lekcja[1], lekcja[3]]]
                    lekcje_z_zastepstwami.append(lekcja)
                    zastempstaw.append(lekcja)

        if not day:
            lekcje_z_zastepstwami.append([lekcja_temp, 'Brak'])

        if day == 'XXX':
            lekcje_z_zastepstwami.append([lekcja_temp, 'XXX'])
...
def zapis():
    for z in lekcje_z_zastepstwami:
        if len(z) < 1:
            lekcje_z_zastepstwami.remove(z)
